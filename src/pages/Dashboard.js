...
function Dashboard({ prayerTimes, userData, tasks, prayerLog, onLogPrayer, error }) {
    ...
    useEffect(() => {
        ...
        const getNextPrayer = () => {
            ...
            .map(([name, time]) => ({ name: PRAYER_NAMES[name], time }))
            .sort((a, b) => a.time.localeCompare(b.time));

            let next = sortedPrayers.find(p => p.time > nowTime);
            let isTomorrow = false;
            if (!next) { // Jika semua shalat hari ini telah berlalu, tampilkan Subuh untuk besok
        ...
        getNextPrayer();
        const interval = setInterval(getNextPrayer, 60000); // Update setiap menit
        return () => clearInterval(interval);
    }, [prayerTimes]);

    const tasksCompletedToday = tasks.filter(t => {
        if (t.status !== 'completed' || !t.completionDate) return false;
        const completionDate = new Date(t.completionDate).toISOString().split('T')[0];
    ...
    return (
        ...
        <h3 className="font-bold text-gray-700">Tugas Selesai Hari Ini</h3>
        <p className="text-3xl font-bold text-teal-600">{tasksCompletedToday}</p>
        </div>
        <div className="bg-white p-4 rounded-xl shadow">
        <h3 className="font-bold text-gray-700">Poin Anda</h3>
        <p className="text-3xl font-bold text-teal-600">{userData?.points || 0}</p>
        </div>
        </div>
        ...
        {prayerTimes ? Object.entries(prayerTimes)
            ...
            .map(([apiName, time]) => {
                ...
                return (
                    ...
                    {log ? (
                        <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm ${log.status === 'dilaksanakan' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {log.status === 'dilaksanakan' ? <CheckCircleIcon /> : <XCircleIcon />}
                        <span>{log.status === 'dilaksanakan' ? 'Sudah' : 'Terlewat'}</span>
                        </div>
                    ) : (
                        <div className="flex gap-2">
                        <button onClick={() => onLogPrayer(displayName, 'dilaksanakan')} className="bg-green-500 text-white p-2 rounded-full shadow hover:bg-green-600"><CheckCircleIcon /></button>
                        <button onClick={() => onLogPrayer(displayName, 'terlewat')} className="bg-red-500 text-white p-2 rounded-full shadow hover:bg-red-600"><XCircleIcon /></button>
                        </div>
                    )}
                    </div>
                )
